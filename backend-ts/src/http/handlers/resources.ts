...
/*******************************************************************************
 * Function: buildWorkflowYamlFromSteps
 *
 * Builds workflow YAML from chat tool steps and the user prompt.
 ******************************************************************************/
...
/*******************************************************************************
 * Function: loadWorkflowsCatalog
 *
 * Loads and caches the workflow catalog and blocked-request patterns.
 ******************************************************************************/
 ...
 *
 * Checks a workflow request against catalog support and blocking rules.
 ******************************************************************************/
 ...
 *
 * Dispatches company, chat, provider, webhook, and integration requests.
 ******************************************************************************/
 ...
 *
 * Checks whether the user has a company-administration role.
 ******************************************************************************/
function isCompanyAdmin(user: CurrentUser): boolean { return user.role === "Platform Admin" || user.role === "System Admin"; }
/*******************************************************************************
 * Function: requireCompanyWrite
 *
 * Requires a company-administration role before changing company data.
 ******************************************************************************/
...
/*******************************************************************************
 * Function: requireCompanyRead
 *
 * Requires a company-administration role before reading restricted company
 * data.
 ******************************************************************************/
 ...
 * Function: company
 *
 * Returns a copy of the stored company profile or its defaults.
 ******************************************************************************/
 ...
 * Function: getCompany
 *
 * Returns company profile data appropriate for the current user.
 ******************************************************************************/
 ...
 *
 * Validates and updates the company profile.
 ******************************************************************************/
...
/*******************************************************************************
 * Function: listCompanyCollection
 *
 * Returns visible items from a company profile collection.
 ******************************************************************************/
 ...
 *
 * Validates and adds an item to a company profile collection.
 ******************************************************************************/
...
/*******************************************************************************
 * Function: updateCompanyItem
 *
 * Validates and updates an item in a company profile collection.
 ******************************************************************************/
 ...
 *
 * Removes an item from a company profile collection.
 ******************************************************************************/
...
/*******************************************************************************
 * Function: parseCompanyItem
 *
 * Validates an item using the schema for its company collection.
 ******************************************************************************/
 ...
 *
 * Formats company validation issues as an HTTP failure with field errors.
 ******************************************************************************/
 ...
 *
 * Reads the identity field used by a company collection item.
 ******************************************************************************/
...
/*******************************************************************************
 * Function: companyItemName
 *
 * Returns the display name for a company collection item type.
 ******************************************************************************/
...
/*******************************************************************************
 * Function: companyItemLabel
 *
 * Returns the lowercase label for a company collection item type.
 ******************************************************************************/
...
/*******************************************************************************
 * Function: companyActionName
 *
 * Returns the audit action name for a company collection item type.
 ******************************************************************************/
 ...
 *
 * Applies defaults to the company profile's settings and collections.
 ******************************************************************************/
 ...
 *
 * Returns chat metadata without the messages array.
 ******************************************************************************/
...
// The frontend only ever reads `artifacts.retrieval` off the LAST message
// that has one (see useChat.js: `[...messages].reverse().find(...)`) — so
// carrying the full tool/rule/example retrieval payload (up to ~100 tool
// records) on every older workflow-generation message in a session is pure
// waste. A chat with several such messages balloons past what Azure's
// ingress will reliably relay in one response (silent truncation ->
// net::ERR_HTTP2_PROTOCOL_ERROR client-side). Strip it from everything but
// the newest occurrence before returning the full session.
/*******************************************************************************
 * Function: trimRedundantRetrieval
 *
 * Removes retrieval entries already represented by validation artifacts.
 ******************************************************************************/
function trimRedundantRetrieval(messages: Array<Record<string, unknown>>): Array<Record<string, unknown>> {
  let lastRetrievalIndex = -1;
  for (let i = messages.length - 1; i >= 0; i--) {
    const artifacts = messages[i]!.artifacts;
    if (isRecord(artifacts) && artifacts.retrieval !== undefined) { lastRetrievalIndex = i; break; }
  }
  return messages.map((message, index) => {
    const artifacts = message.artifacts;
    if (!isRecord(artifacts) || artifacts.retrieval === undefined || index === lastRetrievalIndex) return message;
    const { retrieval: _retrieval, ...rest } = artifacts;
    return { ...message, artifacts: rest };
  });
}
/*******************************************************************************
 * Function: listChats
 *
 * Returns chat sessions visible to the current user.
 ******************************************************************************/
 ...
 * Function: createChat
 *
 * Creates a chat session owned by the current user.
 ******************************************************************************/
 ...
 * Function: updateChat
 *
 * Updates a visible chat session from the request body.
 ******************************************************************************/
 ...
 * Function: deleteChat
 *
 * Deletes a chat session after checking the user's authority.
 ******************************************************************************/
 ...
 *
 * Handles a chat message through the matching query, action, or workflow
 * flow.
 ******************************************************************************/
 ...
 *
 * Selects workflow result details to store with a chat message.
 ******************************************************************************/
function compactWorkflowArtifacts(
...
/*******************************************************************************
 * Function: pickArtifactRecords
 *
 * Selects requested fields from object entries in an artifact collection.
 ******************************************************************************/
...
/*******************************************************************************
 * Function: generateWorkflowNarrative
 *
 * Builds a chat explanation of workflow generation and validation results.
 ******************************************************************************/
 ...
 *
 * Requires the Platform Admin role before provider administration.
 ******************************************************************************/
 ...
 *
 * Returns provider configurations with secret fields removed.
 ******************************************************************************/
 ...
 *
 * Validates and normalizes provider configuration input.
 ******************************************************************************/
 ...
 *
 * Activates the selected provider in storage and the runtime.
 ******************************************************************************/
 ...
 *
 * Tests a stored provider configuration and returns the result.
 ******************************************************************************/
 ...
 *
 * Validates and stores a new webhook configuration.
 ******************************************************************************/
 ...
 *
 * Applies requested changes to an existing webhook.
 ******************************************************************************/
 ...
 *
 * Removes the requested webhook configuration.
 ******************************************************************************/
 ...
 *
 * Sends a test request to the configured webhook endpoint.
 ******************************************************************************/
 ...
 *
 * Validates and stores a new integration configuration.
 ******************************************************************************/
 ...
 *
 * Returns an integration with secret fields removed.
 ******************************************************************************/
 ...
 *
 * Applies requested changes to an existing integration.
 ******************************************************************************/
 ...
 *
 * Removes the requested integration configuration.
 ******************************************************************************/
 ...
 *
 * Checks the configured integration and records its connection status.
 ******************************************************************************/
 ...
 *
 * Summarizes available ERP tools by module for a chat response.
 ******************************************************************************/

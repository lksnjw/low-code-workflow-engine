...
func TestProductionConfigIgnoresExperimentBaselineEnvironment(t *testing.T) {
	t.Setenv("APP_ENV", "development")
	t.Setenv("EXPERIMENT_BASELINE", "B")
	withBaseline := Load()
	t.Setenv("EXPERIMENT_BASELINE", "")
	withoutBaseline := Load()
	if !reflect.DeepEqual(withBaseline, withoutBaseline) {
		t.Fatalf("production configuration changed when EXPERIMENT_BASELINE was set: with=%+v without=%+v", withBaseline, withoutBaseline)
	}
	if err := withBaseline.Validate(); err != nil {
		t.Fatalf("inert experiment variable affected server configuration validation: %v", err)
	}
}
